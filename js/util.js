...
wkApp.util = {
	...
	loading: function(show) {
		...
		if (show) {
			if (++wkApp.util.showCount === 1) loader.setAttribute('indeterminate');
		} else {
			if (--wkApp.util.showCount === 0) loader.removeAttribute('indeterminate');
		}
	},

	waitFor: async function(func) {
		wkApp.util.loading(true);
		try {
			await func();
		} catch(e) {
			wkApp.util.error(e.message);
		}
		finally {
			wkApp.util.loading(false);
		}
	},
