...
wkApp.controllers['page-list-ops'] = {
	...
	validateFields: function(event) {
		...
		if (listCode.length > 0) {
			if (listCode.length !== 4) {
				wkApp.util.error('Quick access code must have exactly 4 characters.');
				return false;
			}
		...
		// Otherwise target list name and PIN
			...
			wkApp.util.error('List name must have at least 10 characters.');
			return false;
		} else if (listPin.length < 4) {
			wkApp.util.error('Access PIN must have at least 4 characters.');
			return false;
		}
		return { page: page, listName: listName, listPin: listPin, listCode: listCode };
	},
	...
	getCode: async function(event) {
		...
		let code = null;
		await wkApp.util.waitFor(async () => { code = await wkApp.services.list.getQuickAccessCode(); });
		if (code === null) return;
		// Display the code
		let page = document.querySelector('#id-navigator').topPage;
	...
	saveSettings: async function(event) {
		...
		if (button == 1) {
			wkApp.util.waitFor(async () => {
				await wkApp.services.list.copy(fields.listName, fields.listPin);
				document.querySelector('#id-navigator').popPage();
			});
		}
	},
	...
	newList: function(event) {
		...
		// Create new list
		wkApp.util.waitFor(async () => {
			await wkApp.services.list.create(fields.listName, fields.listPin);
			document.querySelector('#id-navigator').popPage();
			wkApp.util.alert('New list created');
		});
	},
	...
	openList: function(event) {
		...
		wkApp.util.waitFor(async () => {
			...
			if (fields.listCode.length > 0) {
				let json = await wkApp.services.list.redeemQuickAccessCode(fields.listCode);
				fields.listName = json.listName;
				fields.listPin = json.listPin;
			}
			// Open list with given name and PIN
			await wkApp.services.list.open(fields.listName, fields.listPin);
			document.querySelector('#id-navigator').popPage();
			wkApp.util.alert('List opened');
		});
	},
